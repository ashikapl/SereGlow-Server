...
def get_feedback_store(service_id):
    result = supabase.table("Feedback").select("*").eq("service_id", service_id).execute()
...
def delete_feedback_store(service_id, feedback_id):
    result = supabase.table("Feedback").delete().eq("service_id", service_id).eq("id", feedback_id).execute()
